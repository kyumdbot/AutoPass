...
class StorageHelper {
    ...
    func insertItem(title: String, password: String, appendEnter: Bool) {
        let item = Item(context:self.persistentContainer.viewContext)
        item.title = title
        item.password = password
        item.appendEnter = appendEnter
    }
    ...
    func queryAll() -> [Item]? {
        do {
            let items = try self.persistentContainer.viewContext.fetch(Item.fetchRequest())
